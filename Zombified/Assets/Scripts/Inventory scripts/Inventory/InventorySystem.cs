...
public class InventorySystem : MonoBehaviour
{
    ...
    public delegate void ItemDelegate(InventoryItem item);
    ...
    void Start()
    {
        if (transform.GetComponent<ToolBar>() == null)
    ...
    public void UnEquipItem1(InventoryItem item)
    {
        if (UnEquipItem != null)
            UnEquipItem(item);
    }
    ...
    public void updateItemList()
    {
        ...
        for (int i = 0; i < SlotContainer.transform.childCount; i++)
        {
            ...
            {
                ItemsInInventory.Add(trans.GetChild(0).GetComponent<ItemWithObject>().item);
            }
        }
    ...
    {
        if (GetComponent<EquipmentSystem>() == null && GetComponent<ToolBar>() == null && GetComponent<CraftSystem>() == null)
        {
            height = 5;
            width = 5;
        ...
        else if (GetComponent<ToolBar>() != null)
        {
            height = 1;
            width = 9;
    ...
    public void updateSlotAmount(int width, int height)
    {
        ...
        List<InventoryItem> itemsToMove = new List<InventoryItem>();
        ...
        while (slotList.Count > width * height)
        {
            ...
            ItemWithObject itemInSlot = go.GetComponentInChildren<ItemWithObject>();
            if (itemInSlot != null)
            {
        ...
        {
            foreach (InventoryItem i in itemsToMove)
            {
        ...
        for (int k = 0; k < ItemsInInventory.Count; k++)
        {
            for (int i = 0; i < SlotContainer.transform.childCount; i++)
            {
                if (SlotContainer.transform.GetChild(i).childCount == 0)
                {
                    ...
                    item.GetComponent<ItemWithObject>().item = ItemsInInventory[k];
        ...
        for (int i = 0; i < ItemsInInventory.Count; i++)
        {
            if (ItemsInInventory[i].itemID == itemID)
            {
                ...
                if (stack <= ItemsInInventory[i].maxStack)
                {
                    ...
                    if (temp != null && temp.GetComponent<ConsumingItems>().duplicateItem != null)
                        temp.GetComponent<ConsumingItems>().duplicateItem.GetComponent<ItemWithObject>().item.itemValue = stack;
                    return true;
                }
            }
        }
    ...
    public void addItemToInventory(int id)
    {
        for (int i = 0; i < SlotContainer.transform.childCount; i++)
        {
            ...
            {
                GameObject item = (GameObject)Instantiate(prefabItem);
                item.GetComponent<ItemWithObject>().item = itemDatabase.getItemByID(id);
                item.transform.SetParent(SlotContainer.transform.GetChild(i));
                item.GetComponent<RectTransform>().localPosition = Vector3.zero;
                item.transform.GetChild(0).GetComponent<Image>().sprite = item.GetComponent<ItemWithObject>().item.itemIcon;
                item.GetComponent<ItemWithObject>().item.indexItemInList = ItemsInInventory.Count - 1;
                break;
            }
        }
    ...
    public GameObject addItemToInventory(int id, int value)
    {
        for (int i = 0; i < SlotContainer.transform.childCount; i++)
        {
            ...
            {
                GameObject item = (GameObject)Instantiate(prefabItem);
                ItemWithObject itemOnObject = item.GetComponent<ItemWithObject>();
    ...
    public void stackableSettings(bool stackable, Vector3 posi)
    {
        for (int i = 0; i < SlotContainer.transform.childCount; i++)
        {
            if (SlotContainer.transform.GetChild(i).childCount > 0)
            {
                ItemWithObject item = SlotContainer.transform.GetChild(i).GetChild(0).GetComponent<ItemWithObject>();
    ...
    public List<InventoryItem> getItemList()
    {
        ...
        {
            if (SlotContainer.transform.GetChild(i).childCount != 0)
                theList.Add(SlotContainer.transform.GetChild(i).GetChild(0).GetComponent<ItemWithObject>().item);
        }
        return theList;
    }
    ...
    public GameObject getItemGameObjectByName(InventoryItem item)
    {
        for (int k = 0; k < SlotContainer.transform.childCount; k++)
        {
            if (SlotContainer.transform.GetChild(k).childCount != 0)
            {
                ...
                InventoryItem itemObject = itemGameObject.GetComponent<ItemWithObject>().item;
    ...
    public GameObject getItemGameObject(InventoryItem item)
    {
        for (int k = 0; k < SlotContainer.transform.childCount; k++)
        {
            if (SlotContainer.transform.GetChild(k).childCount != 0)
    ...
    public void deleteItemFromInventory(InventoryItem item)
    {
        for (int i = 0; i < ItemsInInventory.Count; i++)
        {
            if (item.Equals(ItemsInInventory[i]))
    ...
    public void deleteItemFromInventoryWithGameObject(InventoryItem item)
    {
        for (int i = 0; i < ItemsInInventory.Count; i++)
        {
            if (item.Equals(ItemsInInventory[i]))
    ...
    public int getPositionOfItem(InventoryItem item)
    {
        for (int i = 0; i < SlotContainer.transform.childCount; i++)
        {
            if (SlotContainer.transform.GetChild(i).childCount != 0)
            {
                InventoryItem item2 = SlotContainer.transform.GetChild(i).GetChild(0).GetComponent<ItemWithObject>().item;
